...
namespace BriSky.Data.Comercial
{
    ...
    public class ReservaDAO
    {
        ...
        public string LlamarSpCrearReservaInternet(string codReserva, string codPasajero, int idVuelo, string codTarifa, string ipOrigen)
        {
            using (var con = Conexion.GetConnection())
            {
                con.Open();
                using (var cmd = new SqlCommand("brisky.crear_reserva_internet", con))
                {
                    cmd.CommandType = CommandType.StoredProcedure;
                    cmd.Parameters.Add("@p_cod_reserva", SqlDbType.VarChar, 12).Value = codReserva;
                    cmd.Parameters.Add("@p_cod_pasajero", SqlDbType.VarChar, 10).Value = codPasajero;
                    cmd.Parameters.Add("@p_id_vuelo", SqlDbType.Int).Value = idVuelo;
                    cmd.Parameters.Add("@p_cod_tarifa", SqlDbType.VarChar, 10).Value = codTarifa;
                    cmd.Parameters.Add("@p_ip_origen", SqlDbType.VarChar, 45).Value = (object)ipOrigen ?? DBNull.Value;
